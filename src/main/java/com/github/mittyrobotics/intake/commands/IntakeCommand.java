...
public class IntakeCommand extends CommandBase {
    ...
    @Override
    public void execute() {
        if (StateMachine.getInstance().getCurrentRobotState() == StateMachine.RobotState.HP || StateMachine.getInstance().getCurrentRobotState() == StateMachine.RobotState.GROUND)
            IntakeSubsystem.getInstance().setMotor(IntakeSubsystem.getInstance().isIntakeFull() ? 0 : IntakeConstants.INTAKE_SPEED);
        else if (StateMachine.getInstance().getCurrentRobotState() == StateMachine.RobotState.MID || StateMachine.getInstance().getCurrentRobotState() == StateMachine.RobotState.HIGH)
            IntakeSubsystem.getInstance().setMotor(StateMachine.getInstance().readyToShoot() ? IntakeConstants.OUTTAKE_SPEED : 0);
